<template >
    <div ref="pageOkrDetailRef" :class="isSingle ? ['max-h-[100%]'] : ['md:max-h-[640px] md:min-h-[640px]']"
        class="flex flex-col h-full md:h-auto md:flex-row">
                        ...
                        <div class="flex md:items-center">
                            ...
                            <div v-if="detailData.completed == '0' && detailData.canceled == '0' && detailData.end_at">
                                <n-tag class="md:ml-4 mt-4 md:mt-0" v-if="within24Hours(detailData.end_at)" type="info"><i
                                        class="okrfont text-14 mr-4">&#xe71d;</i>{{ expiresFormat(detailData.end_at)
                                        }}</n-tag>
                            ...
                            <div class="flex items-center justify-between mt-4">
                                <div class="flex items-center mr-24">
                                    <div class="flex items-start gap-2 max-w-[104px] h-[26px] overflow-hidden">
                                        <div v-if="showUserSelect && detailData.completed == '0' && detailData.canceled == '0' || item.participant != ''"
                                            class="relative">
        ...
        <div
            ...
            <div class="flex items-center justify-between border-solid border-0 border-b-[1px] border-[#F2F3F5] pb-[11px] md:pb-[15px] md:ml-24 min-h-[36px] bg-white"
                ...
                <i v-if="!isSingle" class="okrfont text-16 cursor-pointer text-[#A7ABB5] hidden md:block n-close"
                    @click="closeModal">&#xe6e5;</i>
            </div>
                            ...
                            <div class="flex flex-col bg-[#fff] px-16 pt-24 rounded-lg"
                                ...
                                <div class="flex flex-col justify-between mt-12">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center">
                                            <div class="flex items-start gap-2 max-w-[104px] h-[26px] overflow-hidden">
                                                <div v-if="showUserSelect && detailData.completed == '0' && detailData.canceled == '0'"
                                                    class="relative">
                                                    <div v-if="userInfo.userid != detailData.userid || detailData.canceled == '1' || detailData.completed == '1' || item.score > -1 || item.superior_score > -1"
                                                        class="absolute top-0 bottom-0 left-0 right-0 cursor-not-allowed z-[2]">
                                                    </div>
    ...
    <WarningPopup v-model:show="WarningShow" :title="OTitle" :content="OContent" @submit="handleSubmit"
        @close="WarningShow = false"></WarningPopup>
</template>
...
const canUpdateScore = ref(false)
...
const handleMark = (id, scores, superior_score, progress,can_update_score) => {
    if (userInfo.userid == detailData.value.userid || (detailData.value.superior_user?.indexOf(userInfo.userid) != -1 && detailData.value.superior_user?.indexOf(userInfo.userid) != undefined)) {
        ...
        canUpdateScore.value = can_update_score
